...
/// <summary>Renders Core definition graphs as deterministic Graphviz DOT text.</summary>
public static class GraphvizDotRenderer
{
    /// <summary>Render a Core definition graph as Graphviz DOT text.</summary>
    public static string Render<TState, TEvent>(DefinitionGraph<TState, TEvent> graph,
        GraphvizRenderOptions? options = null)
    {
        ArgumentNullException.ThrowIfNull(graph);

        var resolvedOptions = options ?? new GraphvizRenderOptions();
        resolvedOptions.Validate();

        var orderedNodes = GraphvizCanonicalOrdering.Nodes(graph.Nodes);
        var idMap = orderedNodes.ToDictionary(n => n.Node.Id, n => n.Identifier, StringComparer.Ordinal);
        var orderedEdges = GraphvizCanonicalOrdering.Edges(graph.Edges, idMap);
        ...
        sb.Append("digraph ").Append(GraphvizEscaper.QuoteIdentifier(title)).Append(" {").Append(nl);
        sb.Append(indent).Append("rankdir=LR;").Append(nl);

        foreach (var node in orderedNodes)
            sb.Append(indent)
                .Append(GraphvizEscaper.QuoteIdentifier(node.Identifier))
                .Append(" [label=")
                .Append(GraphvizEscaper.QuoteIdentifier(node.Node.Label))
                .Append(node.Node.IsTerminal ? ", shape=doublecircle" : ", shape=circle")
                .Append("];")
                .Append(nl);
        ...
        {
            sb.Append(indent)
                .Append(GraphvizEscaper.QuoteIdentifier(edge.SourceIdentifier))
                .Append(" -> ")
                .Append(GraphvizEscaper.QuoteIdentifier(edge.TargetIdentifier))
                .Append(" [label=")
                .Append(GraphvizEscaper.QuoteIdentifier(EdgeLabel(edge.Edge)));
            if (edge.Edge.TriggerKind == GraphTriggerKind.Completion) sb.Append(", style=dashed");
            sb.Append("];")
                .Append(nl);
        }
    ...
    private static void AppendHierarchyLines<TState, TEvent>(
        ...
        IReadOnlyList<GraphvizCanonicalNode<TState>> nodes,
        StringBuilder sb,
        string indent,
        string nl)
    {
        ...
        foreach (var relationship in graph.ParentChildRelationships
            ...
            sb.Append(indent)
                ...
                .Append(GraphvizEscaper.EscapeComment(StateLabel(relationship.ParentState)))
                .Append(" -> ")
                .Append(GraphvizEscaper.EscapeComment(StateLabel(relationship.ChildState)))
                .Append(nl);
        ...
        foreach (var marker in graph.InitialChildMarkers
            ...
            sb.Append(indent)
                ...
                .Append(GraphvizEscaper.EscapeComment(StateLabel(marker.CompositeState)))
                .Append(" => ")
                .Append(GraphvizEscaper.EscapeComment(StateLabel(marker.InitialChildState)))
                .Append(" (leaf: ")
                .Append(GraphvizEscaper.EscapeComment(StateLabel(marker.ResolvedInitialLeafState)))
                .Append(")")
                .Append(nl);
        ...
        foreach (var marker in graph.HistoryMarkers
            ...
            sb.Append(indent)
                ...
                .Append(GraphvizEscaper.EscapeComment(StateLabel(marker.CompositeState)))
                .Append(" mode=")
                .Append(GraphvizEscaper.EscapeComment(marker.HistoryMode))
                .Append(" fallback=")
                .Append(GraphvizEscaper.EscapeComment(marker.FallbackTargetState is null
                    ? "<none>"
        ...
        foreach (var region in graph.Regions.OrderBy(r => StateLabel(r.CompositeState), StringComparer.Ordinal)
            ...
            sb.Append(indent)
                ...
                .Append(GraphvizEscaper.EscapeComment(StateLabel(region.CompositeState)))
                .Append("/")
                .Append(GraphvizEscaper.EscapeComment(region.RegionName))
                .Append(" initial=")
                .Append(GraphvizEscaper.EscapeComment(region.InitialState is null
                    ? "<none>"
                    : StateLabel(region.InitialState)))
                .Append(" members=")
                .Append(region.MemberStates.Count)
                .Append(" history=")
                .Append(GraphvizEscaper.EscapeComment(region.ParallelHistoryMode ?? "None"))
                .Append(" fallback=")
    ...
    private static void AppendMetadataLines<TState, TEvent>(
        DefinitionGraph<TState, TEvent> graph,
        IEnumerable<GraphvizCanonicalNode<TState>> nodes,
        IEnumerable<GraphvizCanonicalEdge<TState, TEvent>> edges,
        StringBuilder sb,
        string indent,
        string nl)
    {
        var graphMetadata = GraphvizMetadataFormatter.Format(graph.Metadata);
        if (graphMetadata.Length > 0)
            sb.Append(indent).Append("// graph-metadata: ").Append(GraphvizEscaper.EscapeComment(graphMetadata))
                .Append(nl);

        foreach (var node in nodes)
        {
            var metadata = GraphvizMetadataFormatter.Format(node.Node.Metadata);
            var actions = GraphvizMetadataFormatter.FormatActions(node.Node.ExitActions.Concat(node.Node.EntryActions));
            if (metadata.Length > 0 || actions.Length > 0)
            {
                ...
                sb.Append(indent)
                    ...
                    .Append("): ")
                    .Append(GraphvizEscaper.EscapeComment(string.Join(" | ", parts)))
                    .Append(nl);
            }
        }
        ...
        {
            var parts = new List<string>();
            var edgeMetadata = GraphvizMetadataFormatter.Format(edge.Edge.Metadata);
            if (edgeMetadata.Length > 0) parts.Add("edge=" + edgeMetadata);

            var eventMetadata = GraphvizMetadataFormatter.Format(edge.Edge.Event.Metadata);
            if (eventMetadata.Length > 0) parts.Add("event=" + eventMetadata);

            var conditionMetadata = GraphvizMetadataFormatter.Format(edge.Edge.Conditions.Metadata);
            if (conditionMetadata.Length > 0) parts.Add("conditions=" + conditionMetadata);

            var actionMetadata = GraphvizMetadataFormatter.FormatActions(edge.Edge.TransitionActions);
