 ## 1. Transition explainability / dry-run API
    ...
    /speckit.specify Add transition explainability and dry-run support to StateMachineLibrary. Consumers should be able to ask a state machine definition or runtime what would happen for a supplied current active state shape and event
  ...
  ## 2. Graph analysis and definition versioning
    ...
    /speckit.specify Add graph analysis and definition versioning support to StateMachineLibrary Core. Consumers should be able to analyze a state machine definition for unreachable states, dead-end non-terminal states, terminal
  ...
  ## 3. Consumer testing helper package
    ...
    /speckit.specify Add an optional StateMachineLibrary.Testing package that provides fluent test helpers for consumers of StateMachineLibrary. The package should make it easy to assert permitted and denied events, transition outcomes,
  ...
  ## 4. Source generator ergonomics and compile-time validation
    ...
    /speckit.specify Improve StateMachineLibrary.SourceGenerators with stronger generated ergonomics and build-time validation. For declarative state machine definitions, generate strongly typed convenience helpers for applying declared
  ...
  ## 5. Runtime convenience APIs
    ...
    /speckit.specify Add runtime convenience APIs to StateMachineLibrary for snapshotting and batch event application. Consumers should be able to export a runtime snapshot containing the current state or active-state shape, active paths,
  ...
  ## 6. Optional application integration adapters
    ...
    /speckit.specify Add optional application integration adapter packages for StateMachineLibrary. Provide a Microsoft.Extensions.DependencyInjection integration package that lets applications register named or typed state machine
